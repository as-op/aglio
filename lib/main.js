...
exports.collectPathsSync = function (input, includePath) {
	...
	input.replace(INCLUDE, function (match, spaces, filename) {
		...
		return paths.concat(exports.collectPathsSync(content, path.dirname(fullPath)));
	});
	return paths;
};
...
exports.getTheme = function (name) {
	if (!name || LEGACY_TEMPLATES.includes(name) || name === 'olio') {
		return require('./aglio-theme-olio/lib/main.js');
	}
...
exports.render = function (input, options, done) {
	...
	if (fs.existsSync(options.theme)) {
		...
		options.theme = 'olio';
	} else if ((options.theme !== 'default') && LEGACY_TEMPLATES.includes(options.theme)) {
	...
	drafter.parse(filteredInput, {}, function (err, res) {
		let theme;
		benchmark.end('parse');
		if (err) {
			err.input = input;
		...
		for (const option of (theme.getConfig().options || [])) {
			// Convert `foo-bar` into `themeFooBar`
			const words = (option.name.split('-').map((f) => f[0].toUpperCase() + f.slice(1)));
		...
		theme.render(res, options, function (err, html) {
...
exports.renderFile = function (inputFile, outputFile, options, done) {
	const render = input => exports.render(input, options, function (err, html, warnings) {
		if (err) {
			return done(err);
		}
