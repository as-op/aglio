...
module.exports = (renderSchema = function (root, dataStructures) {
	...
	switch (root.element) {
		case 'boolean': case 'string': case 'number':
			...
			if ((root.attributes != null ? root.attributes.default : undefined) != null) {
		...
		case 'object': case 'option': {
			...
			while (i < properties.length) {
				...
				if (member.element === 'ref') {
					...
					const exclusive = [];
					for (const option of Array.from(member.content)) {
				...
				if ((member.meta != null ? member.meta.description : undefined) != null) {
					schema.properties[key].description = member.meta.description;
				}
				if (member.attributes != null ? member.attributes.typeAttributes : undefined) {
