...
module.exports = function (base, element) {
	...
	if (element.meta) {
		if (combined.meta == null) { combined.meta = {}; }
		for (key of Object.keys(element.meta || {})) { value = element.meta[key]; combined.meta[key] = value; }
	}
	if (element.attributes) {
		if (combined.attributes == null) { combined.attributes = {}; }
		for (key of Object.keys(element.attributes || {})) { value = element.attributes[key]; combined.attributes[key] = value; }
	}
	...
	if (element.content) {
		if ((combined.content != null ? combined.content.push : undefined) || (element.content != null ? element.content.push : undefined)) {
			// This could be an object or array
			if (combined.content == null) { combined.content = []; }
			for (const item of Array.from(element.content)) {
